...
namespace HW {
	...
	double moveGoal(turtlesim::Pose goal_pose);
...
namespace HW {
	...
	double moveGoal(turtlesim::Pose goal_pose)
	{
		...
		do {
			...
			// To check if Turtle1 is a safe distance from villian turtles
			if ((HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, HW::xturtles[0].pose.x, HW::xturtles[0].pose.y)>(D_TOL)) &&
				(HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, HW::xturtles[1].pose.x, HW::xturtles[1].pose.y) > (D_TOL)) &&
				(HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, HW::xturtles[2].pose.x, HW::xturtles[2].pose.y) > (D_TOL))
				)
			{

				//linear velocity
				vel_msg.linear.x = 0.4*HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, goal_pose.x, goal_pose.y);
				vel_msg.linear.y = 0;
				vel_msg.linear.z = 0;
				//angular velocity
				vel_msg.angular.x = 0;
			...
			{
				// If villian turtles get too close stop

				vel_msg.linear.x = 0;
				vel_msg.linear.y = 0;
				velocity_publisher.publish(vel_msg);

				// Check distance against the wall boundaries
				if ((HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, LOWER_LIMIT, LOWER_LIMIT) > 1.5) &&
					(HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, LOWER_LIMIT, UPPER_LIMIT) > 1.5) &&
					(HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, UPPER_LIMIT, LOWER_LIMIT) > 1.5) &&
					(HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, UPPER_LIMIT, UPPER_LIMIT) > 1.5))
				{
					rotate(degrees2radians(180), degrees2radians(180), 1);
					move(1.0, 1.0, 1);
					sum = sum + 1.0;
				}
				...
				// check next move against walls and villian turtles
				if (((HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, HW::xturtles[0].pose.x, HW::xturtles[0].pose.y) > (D_TOL)) &&
					(HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, HW::xturtles[1].pose.x, HW::xturtles[1].pose.y) > (D_TOL)) &&
					(HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, HW::xturtles[2].pose.x, HW::xturtles[2].pose.y) > (D_TOL))
					)
					&&
					((HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, LOWER_LIMIT, LOWER_LIMIT) > 1.5) &&
						(HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, LOWER_LIMIT, UPPER_LIMIT) > 1.5) &&
						(HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, UPPER_LIMIT, LOWER_LIMIT) > 1.5) &&
						(HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, UPPER_LIMIT, UPPER_LIMIT) > 1.5)))
				{
					...
					if (((HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, HW::xturtles[0].pose.x, HW::xturtles[0].pose.y) > (D_TOL)) &&
						...
						&&
						((HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, LOWER_LIMIT, LOWER_LIMIT) > 0.5) &&
							(HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, LOWER_LIMIT, UPPER_LIMIT) > 0.5) &&
							(HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, UPPER_LIMIT, LOWER_LIMIT) > 0.5) &&
							(HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, UPPER_LIMIT, UPPER_LIMIT) > 0.5)))
					{
						move(1.0, 0.5, 1);
						sum = sum + 0.5;
					}
					...
					rotate(degrees2radians(270), degrees2radians(270), 1);
					if (((HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, HW::xturtles[0].pose.x, HW::xturtles[0].pose.y) > (1.0)) &&
						(HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, HW::xturtles[1].pose.x, HW::xturtles[1].pose.y) > (1.0)) &&
						(HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, HW::xturtles[2].pose.x, HW::xturtles[2].pose.y) > (1.0))
						)
						&&
						((HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, LOWER_LIMIT, LOWER_LIMIT) > 1.0) &&
							(HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, LOWER_LIMIT, UPPER_LIMIT) > 1.0) &&
							(HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, UPPER_LIMIT, LOWER_LIMIT) > 1.0) &&
							(HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, UPPER_LIMIT, UPPER_LIMIT) > 1.0)))
					{
						move(1.0, 1.0, 1);
						sum = sum + 1.0;
					}
		...
		} while (HW::getDistance(turtlesim_pose.x, turtlesim_pose.y, goal_pose.x, goal_pose.y) > DANGER_TOLERANCE);
...
void HWTest::startTest() {
	...
	cout << "--------------------Homework #4----------------------------" << endl << endl << endl;
	cout << "--------------------Turtle Positions ----------------------" << endl;

	for (int z = 0; z < 3; z++)
	{
		cout << HW::tturtles[z].turtlename << " " << z + 1 << " (x,y): (" << HW::tturtles[z].pose.x << "," << HW::tturtles[z].pose.y << ")" << endl;
	}

	for (int h = 0; h < 3; h++)
	{
		cout << HW::xturtles[h].turtlename << " " << h + 1 << " (x,y): (" << HW::xturtles[h].pose.x << "," << HW::xturtles[h].pose.y << ")" << endl;
	}
	...
	while (ros::ok()) {
		...
		int tturtles = MAX_TTURTLES;

		do {

			turtlesim::Pose goal_pose;
			goal_pose.x = HW::tturtles[tturtles - 1].pose.x;
			goal_pose.y = HW::tturtles[tturtles - 1].pose.y;

			DistanceTotal = DistanceTotal + HW::moveGoal(goal_pose);
			TurtlesCatched++;

			if (((HW::turtle1.pose.x < (goal_pose.x + 0.5)) && (HW::turtle1.pose.x >(goal_pose.x - 0.5))) &&
				((HW::turtle1.pose.y < (goal_pose.y + 0.5)) && (HW::turtle1.pose.y >(goal_pose.y - 0.5)))) {

				//This means the turtle captured it. We then kill the turtle

				turtlesim::Kill::Request reqk;
				turtlesim::Kill::Response respk;

				reqk.name = HW::tturtles[tturtles - 1].turtlename;

				// call to kill target turtle
				if (!HW::kClient.call(reqk, respk))
					ROS_ERROR_STREAM("Error: Failed to kill " << reqk.name.c_str() << "\n");
			...
			//Keep calulating and moving to the position of the turtle.
			else {
				while (HW::turtle1.pose.x != goal_pose.x && HW::turtle1.pose.y != goal_pose.y) {
					DistanceTotal = DistanceTotal + HW::moveGoal(goal_pose);
					TurtlesCatched++;
				}
			}
		...
		cout << "Total Distance traveled: " << DistanceTotal << endl;
		cout << "Number of target turtles captured: " << TurtlesCatched << endl;
...
int main(int argc, char **argv) {
	...
	pose_subscriber = nh.subscribe("/turtle1/pose", 10, HW::poseCallback);
